...
scrapping_history_lastID_select_query = "select last_scrapped from scrapping_history where id=(select max(id) from scrapping_history)"
cursorDate.execute(scrapping_history_lastID_select_query)
date_last_scrapped = cursorDate.fetchall()
scrapping_history_lastID_select_query = "select last_scored from scrapping_history where id=(select max(id) from scrapping_history)"
cursorDate.execute(scrapping_history_lastID_select_query)
date_last_scored = cursorDate.fetchall() 
...
for event in events:
    #Get element class name
        if event.get_attribute("class") == "EventDate":
            ...
            if(date_localized > date_last_scored[0][0]) :
